...
@frozen public enum Result3<Success, Failure> {
    ...
    public var isSucessChanged : Bool {
        switch self {
        case .successChanged:
            return true
        default:
    ...
    public var isSucessNoChange : Bool {
        switch self {
        case .successNoChange:
            return true
        default:
    ...
    @inlinable public func map<NewSuccess>(_ transform: (Success) -> NewSuccess) -> Result3<NewSuccess, Failure> {
        switch self {
        case .successChanged(let success):
    ...
    @inlinable public func mapError<NewFailure>(_ transform: (Failure) -> NewFailure) -> Result3<Success, NewFailure> where NewFailure : Error {
        switch self {
        case .successChanged(let success):
    ...
    ///   `.failure`.
    @inlinable public func flatMap<NewSuccess>(_ transform: (Success) -> Result3<NewSuccess, Failure>) -> Result3<NewSuccess, Failure> {
        switch self {
    ...
    /// - Returns: A `Result` instance, either from the closure or the previous
    ///   `.success`.
    @inlinable public func flatMapError<NewFailure>(_ transform: (Failure) -> Result3<Success, NewFailure>) -> Result3<Success, NewFailure> where NewFailure : Error {
        switch self {
        case let .successChanged(success):
            return .successChanged(success)
        case let .successNoChange(success):
...
public extension Result3 where Failure : Error {
    ...
    @inlinable func get() throws -> Success {
        switch self {
    ...
    @_transparent
    init(prevValue:Success? = nil, catching body: () throws -> Success) {
        do {
    ...
    init(prevValue:Success? = nil, catching body: () throws -> Success)  where Success : AnyObject {
        do {
            let newValue = try body()
    ...
    @_transparent
    init(prevValue:Success? = nil, catching body: () throws -> Success)  where Success : LosslessStringConvertible {
        do {
            let newValue = try body()
    ...
    static func sucessBy(equatingPrevValue prevValue : Success?, with newValue:Success)->Result3<Success, Failure>  where Success : Equatable {
...
extension Result3 where Success : Equatable, Failure : Equatable {
...
extension Result3 : Equatable where Success : Equatable, Failure : Equatable {
