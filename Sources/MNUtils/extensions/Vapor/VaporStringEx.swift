...
fileprivate let dlog : MNLogger? = MNLog.forClass("VaporStringEx")
...
extension String /* helper functions relevant to vapor URL requests, params, base64 params and more  */ {
    ...
    func removingPercentEncodingExf(isLogIssues:Bool = false)-> String? {
        ...
        if result?.contains("%") ?? true {
            ...
            var result = self.replacingOccurrences(ofFromTo: consts.PERCENT_DOUBLY_ESCAPED_HINTS, caseSensitive: false)
            result = self.replacingOccurrences(ofFromTo: consts.PERCENT_ESCAPED_HINTS, caseSensitive: false)
    ...
    private func internal_asQueryParamsDictionary(pairsDelimiter pairDelim:String = "&",
        ...
        for part in parts {
            ...
            let consts = MNUtils.constants
            if comps.count != 2 || parts.first == consts.BASE_64_PARAM_KEY {
