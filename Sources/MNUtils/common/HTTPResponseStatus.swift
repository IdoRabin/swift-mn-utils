...
public enum HTTPResponseStatus {
    ...
    case ok
    case created
    case accepted // 202 Accepted response status code indicates that the request has been accepted for processing, but the processing has not been completed; in fact, processing may not have started yet
    ...
    // 4xx
    case badRequest
    case unauthorized // 401 // MDN: 401 Response status code indicates that the client request has not been completed because it lacks valid authentication credentials for the requested resource.
    case paymentRequired
    case forbidden // 403 // MDN 403 The HTTP 403 Forbidden response status code indicates that the server understands the request but refuses to authorize it. Use 410 is the resource is permenantly dead.
    case notFound
    ...
    case loopDetected
    case notExtended
    case networkAuthenticationRequired // MDN: The HTTP 511 Network Authentication Required response status code indicates that the client needs to authenticate to gain network access. This status is not generated by origin servers, but by intercepting proxies that control access to the network.
